...
export function useError() {
  ...
  const setError = (errorCode, fallbackKey = 'unknown-error') => {
    const messages = errorMessages[currentLanguage.value]
    globalError.value = messages[errorCode] || messages[fallbackKey]
    globalSuccessMessage.value = ''
  }

  const setSuccess = (successKey) => {
    const messages = successMessages[currentLanguage.value]
    globalSuccessMessage.value = messages[successKey] || ''
    globalError.value = ''
  }
  ...
  const setLoading = (loadingKey = null) => {
    ...
    if (loadingKey) {
      // Optional: Could set a loading message if needed
      clearMessages()
    }
  }
  ...
  const getLoadingMessage = (key) => {
    return loadingMessages.value[currentLanguage.value][key] || ''
  }
